...
function buildLesson(lesson) {
  ...
  const render = (dataset) => {
    ...
    const table = createTable(
      ...
      dataset.map((entry) => lesson.table.mapRow(entry)),
    );
    ...
    try {
      const model = lesson.train(dataset);
      if (lesson.story) {
        if (typeof lesson.story === 'function') {
          insight.innerHTML = lesson.story(model, dataset);
        } else {
          insight.innerHTML = lesson.story;
        }
      ...
      lesson.draw(canvas, dataset, model);
      canvasContainer.appendChild(canvas);
      summaryContainer.style.color = '#1f2933';
      const summaryNode = lesson.summarize(model, dataset);
      if (summaryNode) {
